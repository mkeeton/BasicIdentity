...
using BasicIdentity.Interfaces;
...
namespace BasicIdentity.Repositories
{
  public class UserStore<T> : IUserStore<User, Guid>, IUserLoginStore<User, Guid>, IUserPasswordStore<User, Guid>, IUserSecurityStampStore<User, Guid>, IUserEmailStore<User, Guid>
  {
    ...
    public virtual Task CreateAsync(User user)
    {
      ...
      return Task.Factory.StartNew(() =>
      {
        ...
        using (IDbConnection connection = CurrentContext.OpenConnection())
          connection.Execute("insert into Users(Id, UserName, PasswordHash, SecurityStamp, Email, EmailConfirmed) values(@Id, @userName, @passwordHash, @securityStamp, @email, @emailConfirmed)", user);
      });
    }
    ...
    public virtual Task<User> FindByIdAsync(Guid userId)
    {
      ...
      {
        using (IDbConnection connection = CurrentContext.OpenConnection())
          return connection.Query<User>("select * from Users where Id = @Id", new { Id = userId }).SingleOrDefault();
      });
    }
    ...
    public virtual Task UpdateAsync(User user)
    {
      ...
      {
        using (IDbConnection connection = CurrentContext.OpenConnection())
          connection.Execute("update Users set UserName = @userName, PasswordHash = @passwordHash, SecurityStamp = @securityStamp where Id = @Id", user);
      });
    }
    #endregion
    ...
    public virtual Task<User> FindAsync(UserLoginInfo login)
    {
      ...
      {
        using (IDbConnection connection = CurrentContext.OpenConnection())
          return connection.Query<User>("select u.* from Users u inner join ExternalLogins l on l.UserId = u.Id where l.LoginProvider = @loginProvider and l.ProviderKey = @providerKey",
    ...
    public virtual Task<IList<UserLoginInfo>> GetLoginsAsync(User user)
    {
      ...
      {
        using (IDbConnection connection = CurrentContext.OpenConnection())
          return (IList<UserLoginInfo>)connection.Query<UserLoginInfo>("select LoginProvider, ProviderKey from ExternalLogins where UserId = @userId", new { userId = user.Id }).ToList();
      });
    }
    ...
    public Task<User> FindByEmailAsync(string email)
    {
      ...
      {
        using (IDbConnection connection = CurrentContext.OpenConnection())
          return connection.Query<User>("select * from Users where lower(Email) = lower(@Email)", new { email }).SingleOrDefault();
      });
    }
